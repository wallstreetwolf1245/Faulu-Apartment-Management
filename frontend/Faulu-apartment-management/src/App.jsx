...
const getBuildingOwnerId = (building) =>
  ...
  building?.createdBy ??
  building?.landlordId ??
  building?.landlordUserId ??
  null

import './App.css'
import { normalizeTenant, enrichTenantsWithLookup } from './Landlord/tenantUtils'

// Ensure Authorization header is attached to api requests
// (workaround for broken interceptor in services/api.js)
...
function App() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (propsRes.success) {
          ...
          const propertiesWithOwner = allProperties.filter(
            p => getBuildingOwnerId(p) !== null
          )

          setProperties(
            currentUser?.id && propertiesWithOwner.length > 0
              ? allProperties.filter(
                  p => String(getBuildingOwnerId(p)) === String(currentUser.id)
                )
              : allProperties
          )
        } else {
      ...
      try {
        ...
        if (tenantsRes.data?.success) {
          ...
          console.error('Fetch tenants failed:', tenantsRes.data?.message)
        }
      } catch (err) {
      ...
      try {
        ...
        if (maintRes.data?.success) {
          ...
          console.error('Fetch maintenance failed:', maintRes.data?.message)
        }
      } catch (err) {
      ...
      try {
        if (selectedPropertyId) {
          const paymentsRes = await api.get(
            `/payments?buildingId=${selectedPropertyId}`
          )

          if (paymentsRes.data?.success) {
            setPayments(paymentsRes.data.data)
          } else {
            console.error('Fetch payments failed:', paymentsRes.data?.message)
          }
        } else {
          // Avoid unscoped payments fetch
          setPayments([])
        }
      } catch (err) {
      ...
      try {
        ...
        if (allUnitsRes.data?.success) {
          setAllUnits(allUnitsRes.data.data)
        } else {
          console.error('Fetch all units failed:', allUnitsRes.data?.message)
        }
      } catch (err) {
    ...
    const fetchUnits = async () => {
      try {
        ...
        if (res.data?.success) {
          ...
          console.error('Fetch units failed:', res.data?.message)
          setUnits([])
        }
      } catch (err) {
  ...
  useEffect(() => {
    if (tenants.length > 0 && (properties.length > 0 || allUnits.length > 0)) {
      const enrichedTenants = enrichTenantsWithLookup(
        tenants,
        properties,
        allUnits
      )
    ...
    try {
      ...
      if (res.success) {
        ...
        if (!options.skipNavigate) {
          setCurrentPage('propertiesList')
        }
      } else {
        console.error('Create property failed', res.error)
      }
    } catch (err) {
    ...
    try {
      const payload = {
        tenantId: newPayment.tenantId
          ? Number(newPayment.tenantId)
          : undefined,

        leaseId: newPayment.leaseId
          ? Number(newPayment.leaseId)
          : null,
      ...
      if (err.response) {
        ...
        if (err.response.data?.message) {
          alert(
            `Failed to create payment: ${err.response.data.message}`
          )
        }
      } else {
        alert(
          'Failed to create payment. See console for details.'
        )
      }
  ...
  // Navigation also closes the mobile sidebar
  ...
  const handleUpdateProperty = async (updatedProperty) => {
    try {
      const res = await buildingService.updateBuilding(
        updatedProperty.id,
        updatedProperty
      )
      ...
      if (res.data?.success) {
        ...
        const merged = {
          ...
          propertyName:
            ...
            updatedTenant.propertyName,
          unitNumber:
            returned.unitNumber ||
            updatedTenant.unitNumber,
          unitType:
            returned.unitType ||
            updatedTenant.unitType,
        }
        ...
        if (
          ...
          String(updatedTenant.previousUnitId) !==
            String(updatedTenant.unitId)
        ) {
          try {
  ...
  const handleUpdateMaintenance = async (updatedMaintenance) => {
    try {
      const res = await api.put(
        `/maintenance/${updatedMaintenance.id}`,
        updatedMaintenance
      )
  ...
  // Assigns an Unmatched M-Pesa payment to a unit
  const handleMatchPayment = async (paymentId, unitId) => {
    try {
      const res = await matchPayment(
        paymentId,
        unitId
      )
      ...
      alert(
        'Failed to match payment. See console for details.'
      )
    }
  }
        ...
        <nav className="sidebar-nav">
          ...
          <div className="nav-section">
            ...
            <button
              ...
              onClick={() =>
                handleNavigate('maintenanceList')
              }
              title="Requests"
            >
              <span className="nav-icon">
